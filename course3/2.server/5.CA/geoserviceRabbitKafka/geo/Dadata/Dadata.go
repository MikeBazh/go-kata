...
var (
	limiter      = ratelimit.New(5, ratelimit.Per(time.Minute)) // 5 запросов в минуту
	mu           sync.Mutex
	requestCount int
	startTime    time.Time
)

// Обертка для функции AskByQuery с добавлением rate limit
func RateLimitedAskByQuery(queryAddr string) (SearchResponse SearchResponse, err error, limited bool) {
	mu.Lock()
	defer mu.Unlock()

	// Проверка времени начала
	if time.Since(startTime) > time.Minute {
		startTime = time.Now() // Сброс времени начала
		requestCount = 0       // Сброс счетчика запросов
	}

	// Получаем токен от ограничителя скорости, это блокирует если лимит превышен
	limiter.Take()

	// Проверка, если превышено 5 запросов за текущую минуту
	if requestCount >= 5 {
		//err := fmt.Errorf("ошибка: превышено количество запросов в минуту для запроса %d", requestCount+1)

		//fmt.Println(err)
		return SearchResponse, nil, true
	} else {
		requestCount++
	}
	SearchResponse, err = AskByQuery(queryAddr)
	return SearchResponse, err, false
}

// Обертка для функции AskByQuery с добавлением rate limit
func RateLimitedAskByGeo(lat, lon string) (GeocodeResponse GeocodeResponse, err error, limited bool) {
	mu.Lock()
	defer mu.Unlock()
	...
	// Проверка, если превышено 5 запросов за текущую минуту
	if requestCount >= 5 {
		return GeocodeResponse, nil, true
	} else {
		requestCount++
	}
	GeocodeResponse, err = AskByGeo(lat, lon)
	return GeocodeResponse, err, false
}
