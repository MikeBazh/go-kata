...
import (
	...
	"go-kata/2.server/5.server_http_api/geoservise_gRPC/internal/controller"
	"go-kata/2.server/5.server_http_api/geoservise_gRPC/internal/services"
	"go-kata/2.server/5.server_http_api/geoservise_gRPC/responder"
	"log"
	"net/http"
	"os"
	"os/signal"
...
func main() {
	...
	if !(protocol == "json-rpc" || protocol == "rpc" || protocol == "grpc") {
		log.Fatalf("Server error: Protocol must be set to 'json-rpc' or 'rpc' or 'grpc'")
	}
